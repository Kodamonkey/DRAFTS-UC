...
def _calculate_dynamic_dm_range(
    ...
    slice_len: int,
    fallback_dm_min: int | None = None,
    fallback_dm_max: int | None = None,
    confidence_scores: Iterable | None = None,
) -> Tuple[float, float]:
    """Compute dynamic DM range based on detected candidates."""

    if (
        not getattr(config, "DM_DYNAMIC_RANGE_ENABLE", True)
...
def save_detection_plot(
    ...
    ax.imshow(img_rgb, origin="lower", aspect="auto")
    ...
    if top_boxes is not None:
        for idx, (conf, box) in enumerate(zip(top_conf, top_boxes)):
            ...
            dm_val_real, _, _ = pixel_to_physical(center_x, center_y, slice_len)
    ...
    if band_suffix == "fullband":
        ...
        if top_boxes is not None:
            for box in top_boxes:
                x1, y1, x2, y2 = map(int, box)
                rect = plt.Rectangle((x1, y1), x2 - x1, y2 - y1, linewidth=2, edgecolor="cyan", facecolor="none")
