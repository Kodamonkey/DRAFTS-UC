...
def _cpu_dedisperse_reference(
    ...
    for i in range(height):
        DM = dm_values[i]
        delays = (
            K_DM_MS * DM * (freq_ds ** -2 - freq_ds.max() ** -2)
...
class TestGpuFreqRefInvariant(unittest.TestCase):
    """SPEC-FREQ-001: GPU kernel must not rely on freq[-1] positional assumption."""

    def test_gpu_freq_ref_uses_max(self):
        """After normalize_frequency_axis(), freq is ascending so freq[-1]==freq.max().
        The GPU kernel computes f_ref_inv2 = float(freq.max())**-2 on the host
        before launch — not freq[-1]**-2 inside the kernel. This test validates
        that for any ascending freq array the host-computed value is identical
        to what freq.max() produces (the only safe form).
        """
        freq_asc = np.linspace(1000.0, 1500.0, 64, dtype=np.float32)
        f_ref_from_max = float(freq_asc.max()) ** -2
        f_ref_from_last = float(freq_asc[-1]) ** -2
        # For ascending arrays these must be identical (invariant guaranteed by SPEC-FREQ-001)
        self.assertAlmostEqual(f_ref_from_max, f_ref_from_last, places=12)

    def test_gpu_freq_ref_diverges_if_unsorted(self):
        """If freq were unsorted (violating SPEC-FREQ-001), freq[-1]**-2 != freq.max()**-2.
        This confirms why the kernel must use freq.max(), not freq[-1].
        """
        freq_unsorted = np.array([1500.0, 1000.0, 1300.0, 1200.0], dtype=np.float32)
        f_ref_from_max = float(freq_unsorted.max()) ** -2
        f_ref_from_last = float(freq_unsorted[-1]) ** -2
        # Unsorted: last element (1200.0) != max (1500.0) → different ref (~56% apart)
        self.assertFalse(np.isclose(f_ref_from_max, f_ref_from_last, rtol=1e-3))
